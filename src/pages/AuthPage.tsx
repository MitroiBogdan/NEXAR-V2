			...
			if (isLogin) {
				...
				if (error) {
					...
					if ((error as any)?.message?.includes("Invalid login credentials")) {
						...
						setError((error as any)?.message || "Eroare necunoscută");
					}
				} else if (data?.user) {
					console.log("✅ Login successful for:", data.user.email);
					console.log(
						"🚨 Tocmai m-am logat cu succes, verific dacă apare vreo eroare după acest punct...",
					);
				...
				if (error) {
					...
					if (
						...
						(error as any)?.message?.includes("User already registered")
					) {
						setError(
							"Acest email este deja înregistrat. Încearcă să te conectezi în schimb.",
						);
					} else if (
						(error as any)?.message?.includes("Password should be at least")
					) {
						setError("Parola trebuie să aibă cel puțin 6 caractere");
					} else if (
						(error as any)?.message?.includes("Unable to validate email") ||
						(error as any)?.message?.includes("invalid")
					) {
						setError(
							"Email-ul introdus nu este valid. Te rugăm să verifici formatul.",
						);
					} else if ((error as any)?.message?.includes("signup_disabled")) {
						setError(
						...
						setError(
							`Eroare la înregistrare: ${
								(error as any)?.message || "necunoscută"
							}`,
						);
					}
				} else if (data?.user) {
